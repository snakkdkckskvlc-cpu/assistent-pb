...
Работает **полностью офлайн** на сервере компании. Шесть функций:
...
4. **Пакетная проверка договоров** — несколько файлов разом: тип каждого
   ...
   сводный DOCX-отчёт.
5. **Вопрос по документу** — сотрудник загружает файл и спрашивает своими
   словами; ответ берётся ТОЛЬКО из этого файла, каждое утверждение с цитатой
   и указанием места, а цитата проверяется по тексту.
6. **Транспорт** — учёт машин, рейсов и топлива, печать путевых листов по
   типовым формам № 3 и 4-С.
