...
// Имя пункта совпадает с тем, что человек увидит ПОСЛЕ нажатия: с заголовком
// страницы и с плиткой на «Сегодня». Раньше три пункта назывались иначе —
// «Проверка текста» вела на «Проверку документа», «Вопрос по файлу» на «Вопрос
// по документу», «Проверить пачкой» на «Пакетную проверку». Из-за этого три
// названия подряд начинаются с «Проверк», и сканировать столбец стало чуть
// труднее — но имя, меняющееся между нажатием и результатом, обходится дороже:
// человек перестаёт верить, что попал куда хотел.
const NAV = [
  { items: [["/", "Сегодня", "today"]] },
  { group: "Документы", items: [
    ["/spellcheck.html", "Проверка документа", "doc"],
    ["/legal.html", "Анализ договора", "scales"],
    ["/ask.html", "Вопрос по документу", "search"],
    ["/batch.html", "Пакетная проверка", "stack"],
...
// Готовность модели, посчитанная последней проверкой здоровья. null — ещё не
// спрашивали. Общая переменная, а не второй запрос: /api/health не кэширован и
// каждый его вызов ходит к Ollama. Дашборд опрашивается раз в пять секунд с
// тридцати рабочих мест — отдельный запрос оттуда дал бы сотни обращений в
// минуту к той самой машине, которая в это время считает.
//
// Побочная выгода важнее экономии: шапка и строка «ИИ свободен» теперь берут
// ответ из ОДНОГО места и не могут противоречить друг другу. Раньше могли: в
// шапке «работать нельзя», ниже «ИИ свободен».
let ИИ_ГОТОВ = null;
...
async function updateHealth() {
  ...
  try {
    ...
    if (data.ok) {
      ...
      // Предупреждение сервера сюда уходило ДОСЛОВНО, и в шапке у секретаря
      // висело «Не установлены модели: hf.co/ai-sage/GigaChat3.1-10B-A1.8B-
      // GGUF:Q4_K_M (ollama pull hf.co/ai-sage/...)» — команда консоли и
      // идентификатор модели. Ветку «готово» редизайн от этого вычистил, а
      // эту пропустил: тогда она была короткой, и разница стала видна только
      // когда сменили модель.
      //
      // Сотруднику нужен один факт: работать сейчас нельзя и чинить это не
      // ему. Точный текст с командой нужен ИТ-администратору и остаётся —
      // в подсказке, откуда его можно скопировать и переслать.
      el.textContent = "⚠ Модель не готова — работать нельзя, сообщите ИТ-администратору";
      el.className = "status err";
      el.title = data.ollama.warning || "Модель недоступна";
    }
  } catch (e) {
    ИИ_ГОТОВ = false;
...
// Длительность словами. Общая, потому что своих было две — в истории и в
// сводке, — и вели они себя по-разному на одном и том же числе: одна
// показывала «382 мин», другая «381 мин 36 с». Оба верны и оба нечитаемы:
// чтобы понять, много это или мало, приходится делить на шестьдесят в уме.
//
// От часа и выше секунды отбрасываются: при таком сроке они не значат ничего,
// а разряды съедают внимание. До минуты, наоборот, показываются целиком —
// разница между «2 с» и «40 с» человеку заметна.
function fmtDuration(sec) {
  if (sec === null || sec === undefined) return "—";
  const всего = Math.round(sec);
