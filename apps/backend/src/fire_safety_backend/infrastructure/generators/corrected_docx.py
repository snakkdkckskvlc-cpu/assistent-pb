...
def _replacements(errors: list[dict]) -> list[tuple[str, str]]:
    ...
    for e in errors:
        ...
        # ПУСТОЕ «стало» — не правка, а удаление слова из договора.
        #
        # LanguageTool отдаёт находку без вариантов замены, когда знает, что
        # слово подозрительно, но не знает, чем его заменить: замерено, что так
        # приходят 10 находок из 27. В corrected_text они отсеивались условием
        # `if before and after`, а здесь стояло только `if not before` — и пара
        # ('заводом-изготовителем', '') доезжала до подстановки, ВЫРЕЗАЯ слово.
        #
        # На пяти настоящих договорах так пропадали «заводом-изготовителем»,
        # «сдачи-приёмки», «с ГОСТ». Текст в интерфейсе и текст в скачанном
        # файле расходились, причём файл был хуже — а именно его отправляют
        # заказчику.
