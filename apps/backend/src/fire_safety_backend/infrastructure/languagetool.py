...
# орфографии и пунктуации — категории STYLE/LOGIC/EXTEND (канцелярит,
# «водянистые» формулировки) в _CATEGORY_TO_TYPE не попадают и просто
# отбрасываются в check() ниже.
#
# GRAMMAR здесь появилась не сразу, и это замер, а не вкус. Раньше её тоже
# отбрасывали — вместе с ней терялось «в течении месяца» вместо «в течение»,
# то есть обычная орфографическая ошибка делового письма, которую больше не
# находил никто: модель её тоже пропускала. Замерено на этом железе:
#
#     польза — на размеченных письмах GRAMMAR добавляет находки, которых
#              нет ни у одной другой категории;
#     шум    — на пяти НАСТОЯЩИХ договорах (грамотный текст) GRAMMAR
#              срабатывает 1 раз на весь набор.
#
# Честная оговорка: в GRAMMAR у LanguageTool лежат и правила согласования
...
# LanguageTool по умолчанию работает вполсилы: запрос без параметров включает
# только «безопасный» набор. В нашей установке 6.6 из 488 правил русского
# 30 помечены picky и 13 выключены совсем (default="off").
#
# Их выключили разработчики LanguageTool из-за шума — но у НАС за словарём
# стоит модель, которая и так пересматривает кандидатов, а спорную правку
# отсеивает проверка формы (pipelines/spellcheck.py). Это ровно та связка, где
# режим полноты на словаре выгоден: пусть лучше предложит лишнее, чем смолчит.
_LEVEL = "picky"

# Выключенные по умолчанию правила, которые включаем поимённо: picky их не
# поднимает, нужен явный список.
#
# Unknown_words — слова, которые орфографический словарь молча пропускает;
# PUNKT_KOTORIJ1 — запятая перед «который»; Prep_i_2 — запятая перед «и»/«как»
# в сложном предложении; COMPLEX_SOUZ_PUNCT_2 — составные союзы;
# PREDLOGI_KROME — обособление «кроме»; Obobchajuchee_slovo — двоеточие при
# обобщающем слове.
#
# НЕ включаем KAVITCHKI и Three_dot: они гонят кавычки к немецкому „…“ и
# портят оформление русского делового документа. MissingYO тоже нет — в
# деловой переписке «ё» не пишут, и правило залило бы каждый документ.
_ENABLED_RULES = ",".join(
    (
        "Unknown_words",
        "PUNKT_KOTORIJ1",
        "Prep_i_2",
        "COMPLEX_SOUZ_PUNCT_2",
        "PREDLOGI_KROME",
        "Obobchajuchee_slovo",
    )
)
