...
_ПОЧЕМУ_НЕЛЬЗЯ = {
    (ЗАКРЫТ, В_РАБОТЕ): "Документ уже закрыт. Заведите новый, если работа возобновилась",
    (ЗАКРЫТ, ЗАВИЗИРОВАН): "Документ уже закрыт — визировать его поздно",
    (ЗАКРЫТ, ОТКЛОНЁН): "Документ уже закрыт — отклонять его поздно",
    (
        ЗАВИЗИРОВАН,
        В_РАБОТЕ,
    ): "Виза уже стоит. Чтобы отправить документ на доработку, отклоните его с причиной",
}


def _нельзя(из_состояния: str, в_состояние: str) -> str:
    """Объяснение отказа человеческим языком.

    Раньше здесь было «Из состояния «завизирован» нельзя перейти в «в работе»».
    Это текст для программиста: он сообщает, ЧТО запрещено, но не говорит, что
    делать. Сообщение читает бухгалтер.
    """
    подсказка = _ПОЧЕМУ_НЕЛЬЗЯ.get((из_состояния, в_состояние))
    if подсказка:
        return подсказка
    return f"Сейчас документ «{из_состояния}» — такое действие для него не подходит"
...
def create(
    *,
    ...
    if kind not in _KIND_CODES:
        raise FlowError("Выберите вид документа из списка")
...
def hand_over(doc_id: int, *, to: str, actor: str, note: str = "") -> None:
    ...
    with connect() as conn:
        ...
        if cur.rowcount == 0:
            raise FlowError(
                "Документ только что изменил кто-то ещё — закройте карточку и откройте заново"
            )
        _log(
            conn,
            doc_id,
...
def change_state(doc_id: int, *, to: str, actor: str, note: str = "") -> None:
    ...
    if to not in ПЕРЕХОДЫ:
        raise FlowError("Выберите действие из списка")
    ...
    with connect() as conn:
        ...
        if to not in ПЕРЕХОДЫ[было["state"]]:
            raise FlowError(_нельзя(было["state"], to))
...
def search(
    *,
    ...
    # Держатели, которых больше нет: уволенный или отключённый сотрудник.
    # Документ у такого не всплывает нигде — он не в чьём-то «у меня», и
    # вопрос «где счёт» снова остаётся без ответа. Помечаем явно.
    with connect() as conn:
        действующие = {
            r["login"] for r in conn.execute("SELECT login FROM users WHERE disabled = 0")
        }
    for doc in rows:
        doc["держатель_потерян"] = bool(
            doc["holder"] and действующие and doc["holder"] not in действующие
        )
        doc["дней_у_текущего"] = (
            0.0
...
def timing(days: int = 90, *, with_people: bool = False) -> dict:
    ...
    def закрыть(конец: str | None) -> None:
        ...
        if текущий and начало_отрезка and конец:
            часы = рабочих_часов(начало_отрезка, конец)
    ...
    for d in закрытые:
        часы = рабочих_часов(d["начало"], d["конец"])
...
# Режим работы компании — из карты информационного потока (строка 5 разбора):
# Пн–Пт, 8:30–17:30, час обеда. Восемь рабочих часов в сутки.
#
# ЗАЧЕМ ЭТО ВООБЩЕ. Отчёт сравнивают с картой, а карта меряет рабочее время.
# Календарный счёт делает цифры несопоставимыми: счёт, отданный финдиректору в
# пятницу в 17:00 и завизированный в понедельник в 9:00, по календарю лежал
# 64 часа, а по факту — полтора рабочих. Показывать «почти трое суток» и
# требовать объяснений было бы неправдой.
РАБОЧИЙ_ДЕНЬ_НАЧАЛО = 8 * 60 + 30
РАБОЧИЙ_ДЕНЬ_КОНЕЦ = 17 * 60 + 30
ОБЕД_МИНУТ = 60
_РАБОЧИХ_МИНУТ_В_ДНЕ = РАБОЧИЙ_ДЕНЬ_КОНЕЦ - РАБОЧИЙ_ДЕНЬ_НАЧАЛО - ОБЕД_МИНУТ


def _минут_в_дне(день: date, с: datetime, по: datetime) -> int:
    """Рабочие минуты одного дня, попавшие в отрезок."""
    if день.weekday() >= 5:  # суббота и воскресенье
        return 0
    начало_дня = datetime.combine(день, time(0)) + timedelta(minutes=РАБОЧИЙ_ДЕНЬ_НАЧАЛО)
    конец_дня = datetime.combine(день, time(0)) + timedelta(minutes=РАБОЧИЙ_ДЕНЬ_КОНЕЦ)
    начало = max(с, начало_дня)
    конец = min(по, конец_дня)
    if конец <= начало:
        return 0
    минут = int((конец - начало).total_seconds() // 60)
    # Обед вычитается пропорционально: точный час выяснять не у кого, а
    # выдумывать его — значит подгонять цифру. Пропорция честнее: у того, кто
    # пробыл полдня, вычтется полчаса.
    полный = РАБОЧИЙ_ДЕНЬ_КОНЕЦ - РАБОЧИЙ_ДЕНЬ_НАЧАЛО
    return max(0, минут - round(ОБЕД_МИНУТ * минут / полный))


def рабочих_часов(с: str, по: str) -> float | None:
    """Рабочие часы между двумя моментами. None — разобрать не удалось.

    Ночь, выходные и обед не считаются. Это делает отчёт сопоставимым с картой
    процессов, которая меряет то же самое.
    """
    try:
        начало, конец = datetime.fromisoformat(с), datetime.fromisoformat(по)
    except (TypeError, ValueError):
        return None
    if конец <= начало:
        return 0.0
    минут = 0
    день = начало.date()
    while день <= конец.date():
        минут += _минут_в_дне(день, начало, конец)
        день += timedelta(days=1)
    return round(минут / 60, 2)
