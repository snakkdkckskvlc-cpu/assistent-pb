...
def search(
    *,
    ...
    if overdue:
        ...
        # Сортировка по ДАВНОСТИ ПОСЛЕДНЕГО ДВИЖЕНИЯ, а не по свежести записи.
        #
        # Раньше стояло ORDER BY id DESC: сверху оказывались только что
        # заведённые документы, а забытый счёт, лежащий третью неделю, уходил
        # в самый низ и при обрезке списка исчезал совсем. Экран существует
        # ровно для того, чтобы такие находились, — значит они и должны быть
        # первыми.
        sql += (
            " ORDER BY COALESCE("
            "  (SELECT MAX(at) FROM doc_flow_event e"
            "   WHERE e.doc_id = doc_flow.id AND e.from_holder <> e.to_holder),"
...
# Меньше минуты — не пребывание, а след регистрации: завёл и сразу передал.
_МИНИМАЛЬНЫЙ_ОТРЕЗОК_ЧАСОВ = 1 / 60


def timing(days: int = 90, *, with_people: bool = False) -> dict:
    ...
    with connect() as conn:
        # Берём ВСЕ события документов, у которых было движение в окне, а не
        # только события внутри окна.
        #
        # Раньше отбор шёл по e.at >= порог, и отрезок, начавшийся до границы,
        # терялся целиком: счёт, попавший к финдиректору за день до порога и
        # пролежавший у него две недели, не давал ни одного отрезка — то есть
        # самое долгое залёживание в отчёт не попадало вовсе. Отбор перенесён
        # на ОКОНЧАНИЕ отрезка (ниже), а выборка расширена до полной истории
        ...
        события = [
            ...
            for r in conn.execute(
                ...
                "WHERE e.doc_id IN (SELECT doc_id FROM doc_flow_event WHERE at >= ?) "
    ...
    def закрыть(конец: str | None) -> None:
        ...
        if текущий and начало_отрезка and конец:
            ...
            # что бы то ни было.
            #
            # Отрезок короче минуты — не пребывание, а след регистрации:
            # секретарь завёл счёт и тут же передал снабженцу. Такие нули
            # тянули медиану человека вниз и делали её неотличимой от нуля у
            # того, кто и правда держит документы днями.
            #
            # Отбор по ОКОНЧАНИЮ отрезка, а не по началу: иначе долгое
            # залёживание, начавшееся до границы окна, в отчёт не попадает.
            if часы is not None and часы >= _МИНИМАЛЬНЫЙ_ОТРЕЗОК_ЧАСОВ and конец >= порог:
    ...
    if not with_people:
        # Разбивка по людям — наблюдение за сотрудниками, а не рабочая
        # информация. То же решение, что на экране «Что происходит»: там она
        # тоже только администратору. Показывать всем тридцати поимённый
        # рейтинг медлительности нельзя: экран про то, где встаёт ПОТОК, и
        # причина чаще не в человеке, а в том, что до него документ дошёл
        # поздно.
        return итог
    итог["у_кого"] = sorted(
        (
