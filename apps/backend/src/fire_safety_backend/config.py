...
# Юр. анализ идёт на GigaChat3.1-10B-A1.8B (MoE, 1,8B активных, лицензия MIT).
# Замерено на датасете из пяти договоров, та же обвязка и тот же промпт:
#
#                         qwen2.5:7b   GigaChat
#     совпало из 36           16          21
...
# уровень разбора. По договорам: F1 0,55 -> 0,79, 0,40 -> 0,80, 0,57 -> 0,89.
#
# СКОРОСТИ MoE НЕ ДАЛА. Прогноз «в 1,5–2,5 раза быстрее» не подтвердился: при
# одинаковом окне 12288 генерация 4,9 ток/с против 5,0 у qwen2.5, то есть
# паритет. Арифметика «активных параметров 1,8B» на CPU не реализуется —
# при чтении промпта задействуются почти все эксперты. Меняем ради КАЧЕСТВА,
# а не ради времени, и время от этого не страдает.
#
# Почему это стало возможно только сейчас: до починки формата ответа GigaChat
# теряла два договора из пяти (на одном отдавала ключи по-английски, на другом
# ломала JSON), и ставить её было нельзя. См. infrastructure/llm.py и
# docs/05-quality/improvements-backlog.md, пункт 2.2.
#
# Орфография ОСТАЁТСЯ на qwen2.5: там десяток коротких запросов на страницу, и
# решает чтение промпта, а не генерация — выигрыш MoE туда не переносится.
LLM_MODEL_LEGAL = (
    os.environ.get("LLM_MODEL_LEGAL") or "hf.co/ai-sage/GigaChat3.1-10B-A1.8B-GGUF:Q4_K_M"
)
LLM_MODEL_SPELLCHECK = os.environ.get("LLM_MODEL_SPELLCHECK") or LLM_MODEL


# Все модели, которые приложение реально использует. Нужен и healthcheck'у, и
# установщику: пока модель была одна, оба обходились LLM_MODEL, а с разными
# моделями на пайплайны это давало зелёный health при отсутствующей модели
# юранализа — задача падала бы уже в работе.
def used_models() -> list[str]:
    """Без повторов и в порядке появления."""
    return list(dict.fromkeys([LLM_MODEL, LLM_MODEL_LEGAL, LLM_MODEL_SPELLCHECK]))
