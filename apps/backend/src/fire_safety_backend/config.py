...
# Модель на конкретный пайплайн. Обе по умолчанию равны LLM_MODEL — разные
# значения нужны там, где задачи разной природы упираются в разное.
#
# Замер и расчёт. Генерация упирается в пропускную способность памяти, чтение
# промпта — в счёт. У MoE (qwen3:30b-a3b) на каждый токен работают 8 экспертов
# из 128, поэтому ГЕНЕРИРУЕТ она быстро, а вот при ЧТЕНИИ промпта в пачке
# задействуются почти все эксперты, и экономия исчезает.
#
# Отсюда разные ответы для разных пайплайнов:
#   юр. анализ  — один длинный запрос, решает генерация → большая модель уместна;
#   орфография  — десяток коротких запросов на страницу, решает чтение промпта
#                 → большая модель тут ХУЖЕ маленькой.
LLM_MODEL_LEGAL = os.environ.get("LLM_MODEL_LEGAL") or LLM_MODEL
LLM_MODEL_SPELLCHECK = os.environ.get("LLM_MODEL_SPELLCHECK") or LLM_MODEL
...
# Ноль по той же причине, что у орфографии: на 40 эталонных рисках разброс
# сэмплирования неотличим от эффекта правок промпта, и сравнивать варианты
# становится нечем. Разбор договора — не творческая задача.
LLM_TEMPERATURE_LEGAL = float(os.environ.get("LLM_TEMPERATURE_LEGAL", "0"))
# Окно орфографии (4096) и юранализа (12288+) НАМЕРЕННО разные, хотя Ollama
# при смене num_ctx выгружает и заново грузит модель — это те самые 9,3 с
# холодного старта при переключении между функциями.
#
# Свести их к общему большому значению замер запрещает: на функции «вопрос по
# документу» окно 12288 вместо 4096 дало 360 с против 163 с, то есть вдвое
# медленнее НА КАЖДОМ запросе. Платить 9,3 с при редком переключении дешевле,
# чем удваивать время каждой проверки орфографии.
