...
def _atomic_edits(before: str, after: str) -> list[tuple[str, str]]:
    ...
    for tag, i1, i2, j1, j2 in difflib.SequenceMatcher(a=b, b=a, autojunk=False).get_opcodes():
        if tag == "equal":
            continue
        # Контекст берётся из одинаковых кусков, поэтому с обеих сторон он один
        # и тот же и на сравнение формы правки не влияет.
        left = max(0, i1 - 1)
        edits.append(
            (
                " ".join(b[left : min(len(b), i2 + 1)]),
                " ".join(a[max(0, j1 - 1) : min(len(a), j2 + 1)]),
            )
        )
...
async def run_spellcheck(
    ...
    (apps/backend/tests/fixtures/spellcheck/), qwen2.5:7b-instruct:
        ...
        LanguageTool + правила    15/31 (48%)     5 с
        вместе с моделью          25/31 (81%)   730 с
    ...
    Размен записан честно: запреты стоят 13 процентных пунктов полноты
    (94% → 81%). Модель находит верную запятую, но кладёт её в один кусок с
    подменой слов рядом, и разделить их удаётся не всегда. Для инструмента,
    которым правят договоры с заказчиком, размен выбран в пользу документа:
    пропущенную запятую человек переживёт, испорченный номер пункта — нет.

    ### Чего этот замер НЕ обещает

    100% не будет. Устойчиво не даются однородные члены без союза и обособление
    причастного оборота. Числа плавают на 1-2 между прогонами даже при нулевой
    температуре. И письма писали мы: это индикатор, а не аттестация.
    """
