...
async def run_ask(
    ...
    # искать.
    task_line = f"ВОПРОС СОТРУДНИКА (отвечать надо на него): {question.strip()}"
    if refined["задание"] and refined["задание"] != question.strip():
        task_line += f"\n\nПодсказка, как понят вопрос: {refined['задание']}"
    if refined["искать"]:
...
__all__ = ["build_blocks", "refine_question", "run_ask", "verify_sources"]
