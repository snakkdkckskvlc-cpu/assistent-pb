...
# Тематический запрос к базе норм под РАЗДЕЛ части договора. Раньше вторым
# запросом всегда шла одна и та же строка про ответственность и неустойку —
# и в контекст части про приёмку ехали ст. 330/394 ГК РФ. Контекст в такой
# ситуации не помогает, а подсказывает: модель писала про санкции там, где
# читала про сроки сдачи, и разделы приёмки, гарантии и споров давали 0
# находок из 8 эталонных.
#
# Правило, а не LLM-роутер, по той же причине, что и в _norm_filter_for:
# лишний вызов модели стоит 20–60 секунд на КАЖДУЮ часть, а выбор здесь
# делается по словам надёжно и бесплатно.
_TOPIC_QUERIES: tuple[tuple[tuple[str, ...], str], ...] = (
    (
        ("приёмк", "приемк", "сдач", "акт выполненных", "скрыт", "мотивированн"),
        "приёмка результата работ акт сдачи скрытые работы отказ от подписания",
    ),
    (
        ("гарант", "недостатк", "дефект", "качеств"),
        "гарантийный срок качество работ недостатки устранение за свой счёт",
    ),
    (
        # «отказ» стеблем, а не оборотом целиком: в договорах это и «отказ от
        # договора», и «вправе отказаться от исполнения», и «односторонний
        # отказ». Проверка по точной фразе мимо второго варианта проходила.
        ("расторж", "отказ", "приостанов", "односторонн"),
        "односторонний отказ от исполнения договора расторжение возмещение затрат",
    ),
    (
        ("спор", "претензи", "арбитраж", "подсудн", "суд"),
        "претензионный порядок разрешение споров подсудность досудебный",
    ),
    (
        ("оплат", "расчёт", "расчет", "аванс", "цена", "смет", "стоимост", "платёж", "платеж"),
        "цена работ смета порядок оплаты аванс изменение цены встречное исполнение",
    ),
    (
        ("срок выполнен", "срок работ", "график", "просрочк", "этап"),
        "сроки выполнения работ начальный конечный срок просрочка исполнения",
    ),
    (
        ("ответственност", "неустойк", "пеня", "пени", "штраф", "убытк", "санкц"),
        "ответственность неустойка штраф убытки ограничение размера",
    ),
)
# Чем закрывается часть, в которой ни одно из слов выше не встретилось.
# Ответственность — самый частый источник рисков, поэтому и была константой.
_DEFAULT_TOPIC_QUERY = "ответственность неустойка штраф убытки"
# Больше двух тематических запросов не берём: итоговый контекст всё равно
# обрезается до _RAG_CHUNKS_PER_PART, а баллы нормализуются ВНУТРИ выдачи
# каждого запроса и между запросами несравнимы — чем больше выдач сливается,
# тем меньше смысла в сортировке по score.
_MAX_TOPIC_QUERIES = 2


def _topic_queries_for(part: str) -> list[str]:
    """Тематические запросы к базе норм под конкретную часть договора.

    Темы отбираются ПО ЧАСТОТЕ упоминаний, а не по порядку в списке. Разница
    не теоретическая: договор целиком задевает все восемь разделов сразу, и
    отбор «первые совпавшие» давал на всех пяти фикстурах одну и ту же пару
    «приёмка + гарантия» — то есть приоритет определялся порядком строк в
    исходнике, а нормы про неустойку исчезали из контекста вовсе. Для
    короткого договора, который не режется на части, это было бы прямым
    ухудшением против прежней константы.

    Счёт по числу вхождений: раздел, которому в тексте посвящено больше
    места, и получает свои нормы.
    """
    low = part.lower()
    weighted = [(sum(low.count(k) for k in keys), q) for keys, q in _TOPIC_QUERIES]
    ranked = sorted((w for w in weighted if w[0] > 0), key=lambda w: w[0], reverse=True)
    return [q for _, q in ranked[:_MAX_TOPIC_QUERIES]] or [_DEFAULT_TOPIC_QUERY]
...
# Ставка неустойки, привязанная к дню просрочки: «2% за каждый день», «0,5 %
# в день», «пеня 1 % ежедневно». Между процентом и указанием на день может
# стоять пояснение («от суммы невыполненных работ»), но не конец предложения.
_DAILY_RATE_RE = re.compile(
    r"(\d+(?:[.,]\d+)?)\s*%[^.;\n]{0,80}?"
    r"(?:за\s+кажд\w+\s+(?:календарн\w+\s+|рабоч\w+\s+)?(?:день|дня|дней|сутки|суток)"
    r"|в\s+день|в\s+сутки|ежедневн)",
    re.IGNORECASE,
)
# Разовый штраф, выраженный в процентах: «штраф в размере 20 % от стоимости».
_FIXED_FINE_RE = re.compile(r"штраф\w*\s+(?:в\s+размере\s+)?(\d+(?:[.,]\d+)?)\s*%", re.IGNORECASE)

# Пороги «красного». Обоснование в цифрах, а не на глаз: законная неустойка по
# ст. 395 ГК РФ — ключевая ставка / 365, это около 0,06 % в день; обычная
# договорная — 0,1 % (36,5 % годовых). 0,5 % в день — это уже 182 % годовых,
# и суды такое штатно снижают по ст. 333 ГК РФ. Разовый штраф от 10 % цены
# договора съедает всю рентабельность подрядных работ.
_RED_DAILY_RATE_PCT = 0.5
_RED_FIXED_FINE_PCT = 10.0
...
def _escalate_severity(finding: dict) -> None:
    """Поднимает критичность до «красного» там, где это следует из цифр.

    Замерено, зачем: на живом прогоне договора ДГ-1010-1548-05-26 модель
    выставила «жёлтый» ВСЕМ 18 находкам из 19 — включая неустойку 2 % за день
    просрочки (730 % годовых) и штраф 20 % от стоимости. Одна из находок даже
    начиналась словами «Критический риск заключается в том, что…» и всё равно
    была помечена жёлтым. Инструкции в промпте такое лечат ненадёжно, поэтому
    два численно проверяемых случая считаются здесь, а не моделью.

    Только повышение: если модель уже сказала «красный», ничего не трогаем.
    Причина повышения записывается в `_критичность_повышена` — пользователь
    должен видеть, что уровень поставила программа, а не модель.
    """
    if finding.get("критичность") == "красный":
        return
    quote = str(finding.get("цитата_из_договора", ""))
    for raw, _ in ((m.group(1), m) for m in _DAILY_RATE_RE.finditer(quote)):
        pct = _percent(raw)
        if pct is not None and pct >= _RED_DAILY_RATE_PCT:
            finding["критичность"] = "красный"
            finding["_критичность_повышена"] = (
                f"неустойка {raw} % за день просрочки — это {pct * 365:.0f} % годовых"
            )
            return
    for m in _FIXED_FINE_RE.finditer(quote):
        pct = _percent(m.group(1))
        if pct is not None and pct >= _RED_FIXED_FINE_PCT:
            finding["критичность"] = "красный"
...
async def run_legal_analysis(
    ...
    for idx, part in enumerate(parts, start=1):
        ...
        # хуже.
        queries = [part[:1500], *_topic_queries_for(part)]
    ...
    # До финального прохода: он получает находки уже с исправленной критичностью
    # и не станет пересказывать «жёлтым» то, что на деле красное.
    for finding in merged_findings:
        _escalate_severity(finding)
