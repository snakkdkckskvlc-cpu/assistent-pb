...
from fire_safety_rag import retrieve
...
async def run_legal_analysis(text: str, task: Task | None = None) -> dict:
    ...
    # Достаём релевантные нормы из RAG. Ключевые запросы:
    ...
    if not context_block:
        context_block = "(нормативная база не подключена — сошлись на общие знания законодательства РФ)"
...
async def run_letter(draft: str, addressee_type: str = "заказчик", task: Task | None = None) -> dict:
    ...
    # Генерируем DOCX на основе фирменного бланка
    ...
    try:
        build_letter_docx(result, output_path)
        result["_docx_path"] = str(output_path.name)
    except FileNotFoundError as e:
        log.warning("Шаблон бланка не найден: %s", e)
        result["_docx_path"] = None
        result["_warning"] = "Фирменный бланк не загружен — сгенерирован только текст письма"
