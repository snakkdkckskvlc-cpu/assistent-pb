...
# --- раздел «Термины и определения» в СП: название термина со строчной ---

_SP_TERMS = """3.1 воздушный затвор: Конструктивный элемент этажного ответвления воздуховода.

3.2 дымоприемное устройство: Проем или отверстие в канале системы вытяжной вентиляции.

3.3 дымовая зона: Часть помещения, защищаемая системами вытяжной вентиляции.
"""


def test_sp_terminology_entries_are_separate_chunks() -> None:
    """Регрессия: раздел терминов слипался в один чанк.

    Правило для СП требует ЗАГЛАВНУЮ букву после номера (чтобы «3.5 м
    составляет…» не стало границей), а термины размечены со строчной. Из-за
    этого весь раздел определений шёл одним куском, чанки СП 2 и СП 7
    выходили втрое крупнее остальных и размывали эмбеддинг: на запрос про
    противодымную вентиляцию первым выдавался СП 5 — свод правил про
    пожарную сигнализацию.
    """
    chunks = chunk_by_articles(_SP_TERMS, 500)
    assert [c["article"] for c in chunks] == ["3.1", "3.2", "3.3"]
    for c in chunks:
        assert (
            sum(t in c["text"] for t in ("воздушный затвор", "дымоприемное", "дымовая зона")) == 1
        )


def test_measurement_line_is_not_a_boundary() -> None:
    """Двоеточие — единственное, что отличает термин от размера в тексте."""
    text = (
        "5.1. Требования к воздуховодам\n"
        "Толщина стенки принимается по расчёту.\n"
        "3.5 м составляет минимальное расстояние до перекрытия.\n"
    )
    articles = [c["article"] for c in chunk_by_articles(text, 500)]
    assert "3.5" not in articles
