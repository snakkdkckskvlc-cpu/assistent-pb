...
#   ППР-1479, полный    → номер пункта ОДИН на строке, текст со следующей:
...
#
# Одноуровневый номер всё-таки ловим, но ТОЛЬКО когда он стоит на строке один:
# у преамбулы СП текст идёт на той же строке, поэтому она под это правило не
# попадает. Без этого полный текст ППР-1479 (261 КБ, ~460 пунктов) давал ШЕСТЬ
# границ на весь документ и уходил обратно на нарезку по словам — самый ходовой
# документ в практике ПБ терял постатейное деление целиком. Замерено на живом
# корпусе: 6 границ → 482, средний чанк 3.6 КБ → 0.5 КБ.
_ARTICLE_BOUNDARY = re.compile(
    ...
    r"|\d+\.\d+(?:\.\d+)*\.?(?=\s+[А-ЯЁA-Z«\"(])"
    r"|\d+(?:\.\d+)*\.[ \t]*(?=\n)"
    # Раздел «Термины и определения» в СП размечен со СТРОЧНОЙ буквы:
    # «3.2 дымоприемное устройство: Проем или отверстие…». Правило выше его
    # не ловит, и весь раздел слипался в один чанк — из-за этого СП 2 и СП 7
    # проигрывали в выдаче соседним документам (на запрос про противодымную
    # вентиляцию первым шёл СП 5 про пожарную сигнализацию). Отличаем от
    # размеров («3.5 м составляет…») по двоеточию: у термина за названием
    # всегда идёт «:», у размера — нет. Проверено на корпусе: 55 попаданий
    # в СП 2/6/7, ложных нет.
    r"|\d+\.\d+(?:\.\d+)*\.?(?=\s+[а-яёa-z][^:\n]{0,80}:)"
    r")",
    re.MULTILINE,
)
